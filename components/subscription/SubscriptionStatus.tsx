...
export function SubscriptionStatus() {
  const { subscription, usage, loading, error, refreshUsage } = useSubscription();
  ...
  // Add debug button for manual refresh
  const handleDebugRefresh = async () => {
    await refreshUsage();
    console.log('[Debug] Refreshed subscription:', subscription);
    console.log('[Debug] Refreshed usage:', usage);
  };
    ...
    <Card className="p-6">
      <div className="space-y-6">
        ...
        <div className="text-center mt-4">
          <Button variant="outline" onClick={handleDebugRefresh}>
            Debug: Refresh Subscription
          </Button>
        </div>
      </div>
    </Card>
