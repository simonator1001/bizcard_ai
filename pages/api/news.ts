...
async function parse(content: string, company: string): Promise<NewsArticle[]> {
  try {
    ...
    // Validate each article with more lenient rules
    const validArticles = parsed.filter(article => {
      ...
      // Check required fields - only title and url are strictly required
      if (!article.title || !article.url) {
        console.warn('[DEBUG] Article missing required fields:', article);
        return false;
      }
      ...
      // Less strict title validation
      if (article.title.length < 5) {
        console.warn('[DEBUG] Title too short:', article.title);
        return false;
      }
  ...
  try {
    ...
    const validArticles = articles.filter(article => {
      ...
      const isRelevant = 
        ...
        article.url.toLowerCase().includes(company.toLowerCase()) ||
        article.source.toLowerCase().includes(company.toLowerCase()) ||
        ...
        article.summary.toLowerCase().includes('company') ||
        article.summary.toLowerCase().includes('corporate');
